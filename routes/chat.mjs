...
import {
  ...
  toChatStartResponseDTO,
  toBerichtResponseDTO,
  toChatResponseDTO,
} from '../dto/chat.dto.mjs';
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!chat) {
      ...
      fetch(process.env.APPS_SCRIPT_URL, {
        ...
        console.error('E-mailnotificatie voor nieuwe chat mislukt:', err)
      );
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(toBerichtResponseDTO(messages));
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!chat) {
      return next(new AppError('Chat niet gevonden', 404));
    }

    res.json(toChatResponseDTO(chat));
  })
);


// ── Berichten tussen 2 users ──
router.get(
  '/messages/:userId',
  isLoggedIn,
...
router.delete(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    res.json(toMessageResponseDTO('Chat verwijderd'));
  })
);
