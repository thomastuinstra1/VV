...
// ── Uitleen ophalen ──
router.get('/uitleen/:id', isLoggedIn, idParamValidator, validate, asyncHandler(async (req, res, next) => {
  const uitleen = await prisma.uitleen.findUnique({
    where: { Uitleen_id: parseInt(req.params.id) }
  });
  if (!uitleen) return next(new AppError('Uitleen afspraak niet gevonden', 404));
  res.json(uitleen);
}));

// ── Uitleen status bijwerken ──
router.patch('/uitleen/:id/status', isLoggedIn, idParamValidator, statusUpdateValidator, validate, asyncHandler(async (req, res, next) => {
  ...
  if (uitleen.Gereedschap.Account_id !== parseInt(req.session.userId)) {
    return next(new AppError('Je hebt geen toegang om de status van deze uitleen te wijzigen', 403));
  }
...
router.get('/dashboard/uitleningen', isLoggedIn, asyncHandler(async (req, res) => {
...
router.get('/dashboard/gereedschap', isLoggedIn, asyncHandler(async (req, res) => {
  const now = new Date();
...
router.get('/mijn-leningen', isLoggedIn, asyncHandler(async (req, res) => {
  const uitleningen = await prisma.uitleen.findMany({
    where:   { Lener_id: req.session.userId },
    include: {
      Gereedschap: {
        include: { Account: { select: { Account_id: true, Name: true, E_mail: true } } }
      }
    },
  ...
  const chatList = await prisma.chats.findMany({
    where: {
      OR: [{ SenderId: req.session.userId }, { ReceiverId: req.session.userId }]
    },
...
router.post('/uitleen', isLoggedIn, uitleenValidator, validate, asyncHandler(async (req, res) => {
