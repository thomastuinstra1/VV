...
function updatePreview() {
  const naam = document.getElementById("Naam")?.value.trim() || "Gereedschap Naam";
  const beschrijving =
    document.getElementById("Beschrijving")?.value.trim() || "Beschrijving gereedschap";
  const borg = document.getElementById("BorgBedrag")?.value || "0";
  const begin = document.getElementById("Begindatum")?.value || "";
  const eind = document.getElementById("Einddatum")?.value || "";

  const previewName = document.getElementById("previewToolName");
  const previewDescription = document.getElementById("previewToolDescription");
  const previewBorg = document.getElementById("previewToolBorg");
  const previewStart = document.getElementById("previewStartDate");
  const previewEnd = document.getElementById("previewEndDate");
  const previewImage = document.getElementById("previewToolImage");
  const gekozenPreview = document.getElementById("gereedschap-preview");

  if (previewName) previewName.textContent = naam;
  if (previewDescription) previewDescription.textContent = beschrijving;
  if (previewBorg) previewBorg.textContent = `Borg: €${borg}`;
  if (previewStart) previewStart.textContent = formatPreviewDate(begin);
  if (previewEnd) previewEnd.textContent = formatPreviewDate(eind);

  if (previewImage) {
    if (gekozenPreview && gekozenPreview.src && gekozenPreview.style.display !== "none") {
      previewImage.src = gekozenPreview.src;
    } else {
...
if (waardeInput && borgInput) {
  ...
  function updateBorg() {
    ...
    if (!borgInput.dataset.edited || huidige < r.min || huidige > r.max || isNaN(huidige)) {
...
if (begindatumInput && einddatumInput) {
  ...
  einddatumInput.addEventListener("change", updatePreview);
}
...
// ==============================
// 🖼️ AFBEELDING PREVIEW + AUTO UPLOAD
// ==============================
const afbeeldingInput = document.getElementById("afbeelding-input");

if (afbeeldingInput) {
  afbeeldingInput.addEventListener("change", async () => {
...
// 🧠 PREVIEW EVENTS
// ==============================
["Naam", "Beschrijving", "BorgBedrag", "Begindatum", "Einddatum"].forEach((id) => {
...
document.getElementById("toolForm").addEventListener("submit", async (e) => {
  ...
  const verplichtGroepen = ["Grootte", "Staat"];
  ...
  try {
    ...
    if (res.ok) {
      ...
      document.getElementById("gereedschap-preview").src = "";
